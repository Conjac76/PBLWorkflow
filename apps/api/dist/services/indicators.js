...
const computeProgressTrajectory = (milestones, artifacts) => {
    ...
    if (!mostRecentMilestone || !mostRecentArtifact)
        return null;
    ...
    if (!(idleMilestoneHours > 48 && idleArtifactHours > 48))
        return null;
...
const computeHelpSeeking = (checkIns) => {
    const candidates = [];
...
const computeMilestoneDriftAndRevision = (milestones, artifacts) => {
    const candidates = [];
    const overdueInProgress = milestones.filter((m) => m.status !== "complete" && m.goalDate && new Date(m.goalDate).getTime() < Date.now());
    ...
    if (activeBaseArtifacts.length > 0) {
        const revisionCounts = activeBaseArtifacts.map((base) => artifacts.filter((a) => a.isRevision && a.parentArtifactId === base.id).length);
...
const computeGamingAndWheelSpinning = (milestones, artifacts, checkIns) => {
    const candidates = [];
...
export const computeIndicatorsForGroup = (groupId) => {
